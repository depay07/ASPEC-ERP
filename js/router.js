// js/router.js - 탭 전환 및 라우팅 (최종 완성본)
...
function renderTabContent(tab, container) {
    ...
    container.innerHTML = `
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-3xl font-bold text-slate-800 border-l-8 border-cyan-500 pl-4">${title}</h2>
            ${buttonsHtml}
        </div>
...
function getTabButtons(tab) {
        ...
        return `
            <div class="flex gap-2">
                <button onclick="openBOMModal()" class="bg-purple-600 hover:bg-purple-700 text-white px-6 py-2.5 rounded font-bold shadow-md text-sm flex items-center gap-2">
                    <i class="fa-solid fa-layer-group"></i> 세트(BOM) 관리
                </button>
    ...
    if (tab === 'inventory') {
        return `
            <button onclick="openNewModal('purchases')" class="bg-green-600 hover:bg-green-700 text-white px-6 py-2.5 rounded font-bold shadow-md text-sm flex items-center gap-2">
                <i class="fa-solid fa-plus"></i> 제품 입고
            </button>`;
    }
...
function getTableStructure(tab) {
    ...
    // 기본 구조 (견적, 주문, 판매)
    const defaultStructure = `<thead><tr><th style="width:10%">일자</th><th style="width:18%">거래처</th><th style="width:12%">담당자</th><th style="width:12%">공급가액</th><th style="width:10%">부가세</th><th style="width:12%">합계</th><th>비고</th><th style="width:15%">관리</th></tr></thead>`;
